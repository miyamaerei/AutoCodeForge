...
description: 'Execution skill: wire Vue 3 frontend modules to .NET backend endpoints, one module per round. Uses fe-be-contract-map as the authoritative gap reference. Handles path fixes, DTO alignment, auth header injection, and mock-to-real migration.'
argument-hint: 'Module name to integrate (e.g. "task-center", "auth", "console"). Omit to auto-select next by priority.'
---
...
- `.github/skills/fe-be-contract-map/SKILL.md`
...
After successful verification, update `.github/skills/fe-be-contract-map/SKILL.md`:
