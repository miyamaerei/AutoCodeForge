...
onMounted(async () => {
  ...
  error.value = ''
  try {
    await store.loadConfigs('Review' as ConfigType)
    const configs = store.getConfigs('Review' as ConfigType)
...
const handleSave = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    error.value = ''
    try {
      await store.saveConfig('Review' as ConfigType, {
        configKey,
