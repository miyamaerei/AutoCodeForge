...
| 需求编号 | 标题 | 说明 | 验收标准 |
|---------|------|------|---------|
| REQ-GIT-AZUREDEFAULT-001 | Azure 默认仓库配置与 Git 集成验证 | 在后端配置中新增 `azureDefault` 配置节，包含 `Token`、`Org`、`Project`、`Repo` 四个字段；基于该 token 对仓库拉取、Git 常用命令和 PR 创建链路进行可执行验证。 | 1) 三套 appsettings 均包含 `azureDefault` 且 JSON 合法；2) 能完成仓库拉取流程验证并输出结果；3) 能验证分支/提交/PR 查询命令；4) 能验证创建 PR 接口行为（成功或可解释失败）。 |
